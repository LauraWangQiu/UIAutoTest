...
class GraphIO:
            ...
            with open(graph_file, "r") as f:
                ...
                for line in f:
                    ...
                    if parts[0] == 'v' and len(parts) == 3:
                        # Vertex definition: exactly 3 parts: v <NAME> <IMAGE_PATH>
                        name = parts[1]
                        ...
                        # Edge definition: 
                        # - CLICK         : e CLICK <SRC> <TGT> <IMG>
                        # - DOUBLE_CLICK  : e DOUBLE_CLICK <SRC> <TGT> <IMG>
                        # - CLICK_AND_TYPE: e CLICK_AND_TYPE <SRC> <TGT> <IMG> <TEXT> 
                        # - DRAG_AND_DROP : e DRAG_AND_DROP <SRC> <TGT> <DRAG_IMG> <DROP_IMG>
                        action_str = parts[1]
                        ...
                        if not src_node or not tgt_node:
                            print(f"[ERROR] Source or target node not found for edge: {line}")
                            continue

                        try:
                            action = ActionType.from_string(action_str)
                        except KeyError:
                            print(f"[ERROR] Unknown action {action_str} in graph file")
                            continue

                        # CLICK y DOUBLE_CLICK 5 tokens
                        if action in (ActionType.CLICK, ActionType.DOUBLE_CLICK):
                            if len(parts) != 5:
                                print(f"[ERROR] Invalid number of args for {action_str}, expected 5 but got {len(parts)}")
                                continue
                            img = parts[4].strip('"')
                        ...
                        # CLICK_AND_TYPE 6 tokens
                        elif action == ActionType.CLICK_AND_TYPE:
                            if len(parts) != 6:
                                print(f"[ERROR] Invalid number of args for CLICK_AND_TYPE, expected 6 but got {len(parts)}")
                                continue
                            img  = parts[4].strip('"')
                            text = parts[5].strip('"')
                            self.handle_click_and_type_action(src_node, tgt_node, text,os.path.join(img_dir, img))

                        # DRAG_AND_DROP 7 tokens
                        elif action == ActionType.DRAG_AND_DROP:
                            if len(parts) != 7:
                                print(f"[ERROR] Invalid number of args for DRAG_AND_DROP, expected 7 but got {len(parts)}")
                                continue
                            ...
                            print(f"[ERROR] Unsupported action type: {action_str}")
    ...
    def handle_double_click_action(self, src_node, tgt_node, image_path):
        """
        Handles the DOUBLE_CLICK action
        """
        transition = self.graph.add_transition(src_node, tgt_node)
        if not transition:
            print("[WARN] add_transition returned None. The transition was not created")
    ...
    def handle_click_and_type_action(self, src_node, tgt_node, text_to_type, image_path):
        """
        Handles the CLICK_AND_TYPE action
        """
        transition = self.graph.add_transition(src_node, tgt_node)
        if not transition:
            print("[WARN] add_transition returned None. The transition was not created")
    ...
    def handle_drag_and_drop_action(self, src_node, tgt_node, drag_image, drop_image):
        """
        Handles the DRAG_AND_DROP action
        """
        transition = self.graph.add_transition(src_node, tgt_node)
        if not transition:
            print("[WARN] add_transition returned None. The transition was not created")
    ...
    def write_graph(self, graph_file, graph):
        """
        Writes the graph definition to the file using the updated edge formats,
        with debug prints for full traceability.
        """
        print(f"[DEBUG] Entered write_graph: graph_file='{graph_file}', nodes={len(graph.nodes)}")
        try:
            print(f"[DEBUG] Opening file '{graph_file}' in write mode...")
            with open(graph_file, "w") as f:
                ...
                for node in graph.nodes:
                    ...
                    print(f"[DEBUG] ├─ Writing vertex: name='{node.name}', image='{image_name}'")
                    f.write(f'v {node.name} {image_name}\n')

                    # Write transitions for this node
                    for trans in node.transitions:
                        ...
                        if act in ("CLICK", "DOUBLE_CLICK"):
                            ...
                            f.write(f'e {act} {node.name} {dst} {image_name}\n')

                        elif act == "CLICK_AND_TYPE":
                            image_name = os.path.basename(trans.image) if trans.image else "None"
                           
                            f.write(f'e {act} {node.name} {dst} {image_name} {trans.text}\n')
                            ...
                            f.write(f'e {act} {node.name} {dst} {drag_image_name} {drop_image_name}\n')

                        else:
                            
                            f.write(f'e {act} {node.name} {dst}\n')

                print(f"[DEBUG] Finished writing vertices and transitions, closing file.")
        except Exception as e:
            print(f"[ERROR] Exception while writing the graph: {e}")
            sys.exit(1)
        else:
            print(f"[DEBUG] write_graph completed successfully and the file was saved at '{graph_file}'.")
